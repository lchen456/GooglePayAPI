...
//    }
    ...
    private void possiblyShowGooglePayButton() {
        ...
        final Optional<JSONObject> isReadyToPayJson;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            isReadyToPayJson = Optional.ofNullable(PaymentsUtil.getIsReadyToPayRequest());
        }
        else{
            isReadyToPayJson=null;
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
...
//            }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            paymentDataRequestJson = Optional.ofNullable(PaymentsUtil.getPaymentDataRequest(itemCents));
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            if (!paymentDataRequestJson.isPresent()) {
                return;
